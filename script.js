...
/**
 * Global Utilities for Airport Lost & Found
 */
...
function validateForm(formId) {
    ...
    requiredInputs.forEach(input => {
        if (!input.value.trim()) {
            isValid = false;
            input.classList.add('glass-input--error');
            setTimeout(() => input.classList.remove('glass-input--error'), 3000);
        }
    });
...
function validateMatch() {
    const lostId = document.querySelector('input[name="lost_id"]:checked');
    const foundId = document.querySelector('input[name="found_id"]:checked');
    
    if (!lostId || !foundId) {
        alert('Please select both a Lost Item and a Found Item to match.');
        return false;
    }
    return confirm('Match these items? This will update their status to "Matched".');
}
